...
@Serializable
class KotlinSerializationGameHighlightResponse(
    val highlights: Highlights = Highlights()
) {
    @Serializable data class Highlights(val highlights: Highlight? = Highlight(), val live: Highlight? = Highlight(), val scoreboardPreview: Highlight? = Highlight())
    @Serializable data class Highlight(val items: List<HighlightItem>? = listOf())
    @Serializable data class HighlightItem(
        val type: String, //video
        val state: String, //A
//        val date: @ContextualSerialization DateTime, //"2018-09-14T19:05:00-0400"
